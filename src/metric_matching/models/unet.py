...
from .unet_utils.fp16_util import convert_module_to_f16, convert_module_to_f32
from .unet_utils.nn import (
    avg_pool_nd,
    checkpoint,
    conv_nd,
    linear,
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        use_new_attention_order=False,
        out_bias=True,  # essential for metric matching training loss to go down. TODO: investigate alternatives/better options
    ):
        super().__init__()
        ...
        self.out_bias_bool = out_bias
        if self.out_bias_bool:
            self.out_bias = nn.Parameter(th.randn(out_channels) * 1e-3)
    ...
    def forward(self, t, x, y=None):
        ...
        if len(x.shape) != 4:
            x = x.reshape(
                x.shape[0], self.in_channels, self.image_size[0], self.image_size[1]
            )
        ...
        h = h.type(x.dtype)
        out = self.out(h)
        # add bias, broadcasting across spatial dims if present
        if out.dim() == 2:
            if self.out_bias_bool:
                return out + self.out_bias.view(1, -1).type(out.dtype)
            return out
        else:
            shape = [1, -1] + [1] * (out.dim() - 2)
            if self.out_bias_bool:
                return (out + self.out_bias.view(*shape).type(out.dtype)).reshape(
                    x.shape[0], self.out_channels, *x.shape[2:]
                )
            return out.reshape(x.shape[0], self.out_channels, *x.shape[2:])
